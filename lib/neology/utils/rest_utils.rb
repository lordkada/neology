module Neology
  ...
  module RestUtils
    ...
    def self.clear_db start_node
      ...
      node_rels = Neology::NeoServer.get.get_node_relationships(start_node)

      if node_rels

        node_rels.each do |rel|

          Neology::NeoServer.get.delete_relationship rel
      ...
      def self.find_node property, value, start_node_id = 0, max_depth= 1
        ...
        start_node = Neology::NeoServer.get.get_node start_node_id

        if start_node

          filter = "position.endNode().hasProperty('#{property}') && position.endNode().getProperty('#{property}') == '#{value}';"

          traverser = Neology::NeoServer.get.traverse(start_node, "nodes", {
                  "order"         => "breadth first",
    ...
    def self.exists_relationship_between? first_node, type, second_node
      ...
      Neology::NeoServer.get.traverse(first_node, "nodes", {
              "order"         => "depth first",
    ...
    def self.follow_and_return_nodes node, relationship, limit=0
      ...
      out_rel = Neology::NeoServer.get.get_node_relationships(node, "out", relationship)
      ...
      if out_rel
        ...
        out_rel.find_all do
          ...
          Neology::NeoServer.get.get_node rel["end"]
